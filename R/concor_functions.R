...
#while(length(axc)!=1 & (axc[1]==1 | axc[1]==0) )
repeat
{   
i=i+1;
...
#zcoors=loc(xc[,+]=0); /* will be isolates */
  ...
  newpart=matrix(NA,nrow(xc),1);
   ...
   while (b<ngrps) { # /* bredth loop: number of parts at this depth,
     ...
     b=b+1;
      ...
      #/* rename the values to make them unique accross partitions */
...
partition=tpart[,p]
part.scaled=as.numeric(factor(partition,labels=rank(unique(partition))))
tpart[,p]=part.scaled
 }
